    ...
    while not done:
        ...
        for i in range(joystick_count):
            ...
            if monitor.mode == 0:
                ...
                if joystick_axis[2]**2 + joystick_axis[1]**2 >= 0.03 ** 2 :
                    monitor.controller["y"] = -4000.0 * controller_curve(joystick_axis[1])  # 前后
                    monitor.controller["x"] = 4500.0 * controller_curve(joystick_axis[2])  # 左右
                else :
                    monitor.controller["y"] = 0.0  # 前后
                    monitor.controller["x"] = 0.0  # 左右
                
                if joystick_axis[0]**2 + joystick_axis[3]**2 >= 0.03 ** 2 :
                    monitor.controller["z"] = 6000.0 * controller_curve(joystick_axis[3]) # 上下
                    monitor.controller["yaw"] = -200.0 * controller_curve(joystick_axis[0])  # 旋转
                else : 
                    monitor.controller["z"] = 0.0 # 上下
                    monitor.controller["yaw"] = 0.0  # 旋转
                #BlueRov机械爪舵机角度 0.0-0.48   
                if(joystick.get_axis(5) != -1):
                    ##机械爪线性变化
                    #monitor.controller["servo0"] = 0.5 * (joystick.get_axis(5) + 1 ) # 舵机
                    monitor.controller["servo0"] = 0.4
                    pass
                elif(joystick.get_button(9)):
                    monitor.controller["servo0"] = 0.0
                elif(joystick.get_button(10)):
                    monitor.controller["servo0"] = 0.48
